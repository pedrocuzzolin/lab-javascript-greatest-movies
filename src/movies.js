...
function howManyMovies(moviesArray) {
    const steveDrama = moviesArray.filter(movie => movie.director === "Steven Spielberg" && movie.genre.includes("Drama"));
...
function dramaMoviesScore(moviesArray) {
    ...
    const totalScore = dramaMovies.reduce((acc, movies) => {
            if (typeof movies.score === 'number') {
                return acc + movies.score;
            } else {
                return acc;
...
function orderByYear(moviesArray) {
    console.log('Cambio para probar')
}
